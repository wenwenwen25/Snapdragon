...
df1= read.csv("./rename_WT_FLEmbedding.csv")
df1[1:4,]
df1=df1[,-1]
colnames(df1)<- c("FA1", "FA2")
df1[1:4,]

df2= read.csv("./WT_TOSICA_prediction_meta_table.csv")
df2[1:4,]
df2= df2[,-1]
df2[1:4,]
dim(df2)
...
pdf("./plots_WT/WT_FA_embedding.pdf", width = 10, height = 10) 
ggplot(df, aes(x=FA1, y=FA2, color=cell_type))+
  geom_point(size=0.5, alpha=0.7, shape=20)+
